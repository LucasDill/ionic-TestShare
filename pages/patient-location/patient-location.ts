...
export class PatientLocationPage {
  ...
  async getWeather() {
    ...
    this.weatherService
      .getWeatherFromApi(this.Data.lat, this.Data.lng)
      .subscribe(weather => {
        ...
        this.Data.origin_weatherdata = [
          this.id,
          this.description,
          this.icon,
          this.tempreal,
          this.tempfeel
        ]; //Set the custom array in the data provider with the weather data
        ...
        this.Data.origin_icon =
          "./assets/weather/" + this.Data.origin_weatherdata[2] + ".png";
  ...
  search($event) {
    ...
    if (q.length > 0) {
      // if there is something there call the search function
      this.Medical_Centers = this.Mapping.SearchCenters(q);
    } else {
      //If there is no results instead of displaying everything display no results
      this.Medical_Centers = [];
    }
  }

  GoNext(
    center //go to the next page if it is the city page or the locations
  ) {
  ...
  async getLatLng(name) {
    ...
    (await this.weatherService.getTimeZone(
      name.lat.toString(),
      name.lng.toString()
    )).subscribe(Results => {
      ...
      async function waitforResults() {
        ...
        if (
          result == undefined ||
          result.dstOffset == undefined ||
          result.rawOffset == undefined
        ) {
          // if anything is undefined the results will loop until they are filled
    ...
    function waitForTimeZone() {
      if (SendingTimeZone == undefined) {
        // wait until the sending time zone variable has been filled
        ...
        if (page.Data.UserTimeZone == SendingTimeZone) {
          ...
          if (Telestroke == true) {
            //if the center entered is a telestroke site go to the Imaging required page and if not go to the imaging routes page
            page.navCtrl.push(ImagingRequiredPage);
          } else {
            page.navCtrl.push(ImagingPage);
          }
        } else if (SendingTimeZone != undefined) {
          // if it is not the same time zone and not undefined call the popup and send in if the function is a telestroke site
  ...
  async TimeZonePopup(Telestroke) {
    // this function will create a popup that asks about the time zones
    var difference = Math.abs(
      Math.abs(this.Data.PatientTimeZone) - Math.abs(this.Data.UserTimeZone)
    ); // find the total difference and add it to the time
    let alert = this.alertController.create({
      title: "Different Time Zones",
      message:
        "The site you have specified is in a different time zone.<br/><br/>Did you enter the last known well in your time zone or the time zone of the sending location?",
      buttons: [
        {
          ...
          handler: () => {
            ...
            if (
              this.Data.UserTimeZone > this.Data.PatientTimeZone &&
              this.Data.HoursSince - difference < 0
            ) {
              // if the time would be negative ////////////////////////////NEED TO CHANGE WAS SHOWING UP WHEN TIME WOULD BE GREATER
              this.WrongTime();
            } else {
  ...
  async WrongTime() {
    let alert = await this.alertController.create({
      ...
      message:
        "The time provided is incorrect with your time zone.\n Would you like to enter a new time?",
      buttons: [
        {
          text: "No"
        },
